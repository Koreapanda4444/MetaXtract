...
def export_case(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        session, records = report.index_store.split_session_and_records(
            index_path
        )
        for fmt in extra_formats:
            if fmt == "html":
                ...
                with open(
                    os.path.join(tmpdir, f"report.{fmt}"),
                    "w",
                    encoding="utf-8"
                ) as f:
                    f.write(html)
        ...
        with open(
            os.path.join(tmpdir, "hashes.txt"),
            "w",
            encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(tmpdir, "meta_manifest.json"),
            "w",
            encoding="utf-8"
        ) as f:
